...
public class ExtentTestNGIReporterListener implements IReporter {
			...
			for (ISuiteResult r : result.values()) {
				...
				if (resultNode != null) {
					...
					resultNode.getModel().setDescription(
							String.format("Pass: %s ; Fail: %s ; Skip: %s ;",
	...
	private void init() {
		...
		htmlReporter.config().setDocumentTitle("MDAutospace-Restful-API自动化测试报告");
		htmlReporter.config().setReportName("MDAutospace-Restful-API自动化测试报告");
	...
	private void buildTestNodes(ExtentTest extenttest, IResultMap tests,
		...
		if (tests.size() > 0) {
			...
			for (ITestResult result : treeSet) {
				...
				/*
				 * Object[] parameters = result.getParameters(); for(Object
				 * param:parameters){ name+=param.toString(); }
				 */
				if (name.length() > 0) {
					if (name.length() > 50) {
